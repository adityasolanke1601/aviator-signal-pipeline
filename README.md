...
2. **`enrich.py`** — pulls an org's repos, computes total monorepo size,
   ...
   a 0-100 fit score. Auto-retries on dropped connections instead of failing
   the whole run over one flaky request.
3. **`outbound.py`** — feeds those signals into an LLM with a system prompt
   that hard-bans generic personalization and requires every claim in the
   email to be traceable to the signal data. Provider-agnostic by design —
   swap between Gemini and Claude with one env var, no code changes.
4. **`main.py`** — CLI that chains discover → enrich → outbound and ranks
   results by fit score into a CSV (drop-in ready for a CRM import). Skips
   and logs any org that fails instead of crashing the whole batch.
...
`results.csv` in this repo is a real, unedited run against 10 auto-discovered
orgs (>5,000 stars, TypeScript) — ranked top to bottom by fit score, with a
generated email for each.
...
- **`gemini`** (default, used for the `results.csv` in this repo) — free, no
  card, get a key at aistudio.google.com/apikey. Good for iterating without
  spending anything.
- **`claude`** — the JD explicitly lists "Claude API" as part of Aviator's
  stack. Swapping to it is a one-line env change; cost is a few cents for a
  full batch (Sonnet 5 pricing), and new Anthropic accounts get $5 in free
  trial credit.

Built it this way on purpose: a real GTM engineering pipeline shouldn't be
...
> top decile into AI-generated outbound that cites the actual repo and PR
> volume instead of a merge tag. That gets a scored, ranked, personalized
